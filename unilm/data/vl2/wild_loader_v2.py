...
CREDENTIAL="sv=2021-10-04&st=2023-06-09T17%3A26%3A48Z&se=2024-01-27T18%3A26%3A00Z&sr=c&sp=rl&sig=7Y6GQgHvWxUZd2yH2dhN7wEqZPK0rrzKjVwi5Ns3AX8%3D"
...
class WildLoader(VLBaseLoader):
    def _setup(self):
        ...
        self.wild_image_from_local = self.args.wild_image_from_local
    ...
    def _get_remote_image_token(self, image_file_name, offset):
    ...
    def _get_local_image_token(self, base64_image):
        pil_img = Image.open(io.BytesIO(base64.b64decode(str(base64_image)))).convert("RGB")
        torch_tensor = self.image_transform(pil_img)
        # TODO: need to change to other format if torch tensor blocks the data loader
        return torch_tensor
    ...
    def _read_from_files(self, source_file):
        ...
        for doc_jsonstr in lines:
            ...
            for image_idx, image_item in enumerate(json_obj[IMAGE_KEY]):
                ...
                try:
                    if self.wild_image_from_local:
                        image_token = self._get_local_image_token(image_item['image_base64'])
                    else:
                        image_token = self._get_remote_image_token(image_item['ImageSubpartitionFilename'], image_item['ByteOffset'])

...
class UniLMModel(BaseFairseqModel):
    ...
    def forward(self, src_tokens=None, tgt_tokens=None, incremental_state=None, classification_head_name=None, mode='both', **kwargs):
        x, extra = self.body(src_tokens, tgt_tokens, incremental_state, return_all_hiddens=True, mode=mode)
...
class UniLMBody(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        if mode == 'both':
            ...
            tokens = src_tokens
            positions = src_positions
            self_attn_mask = self.build_self_attn_mask(src_tokens, bidirectional=False)
...
class UniLMTaskMoeLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if mode == 'both':
            x1 = self.activation_fn(self.fc1[0](x))
            x1 = self.activation_dropout_module(x1)
            x1 = self.fc2[0](x1)

            x2 = self.activation_fn(self.fc1[1](x))
...
class UniLMLayer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if utils.safe_getattr(cfg, "deepnorm_encoder", False):
            self.shortcut_scale = math.pow(2.0 * cfg.encoder_layers, 0.25)
        else:
            self.shortcut_scale = 1.0

        if utils.safe_getattr(cfg, "deepnorm_encoder", False):
            self.fixup_scale = math.pow(8.0 * cfg.encoder_layers, 0.25)
            self.deepnorm_init()
    ...
    def residual_connection(self, x, residual):
        return residual * self.shortcut_scale + x

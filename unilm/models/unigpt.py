...
from unilm.models.connector import build_connector, build_connector2
...
class UniGPTModelConfig(GPTModelConfig):
    ...
    # parameters for visual in sam 
    sam_sample_mode: str = field(
        default="grid",
        metadata={"help": "how to sample the point prompts"},)
    sample_grid_points: int = field(
        default=256,
        metadata={"help": "number of sampled point prompts"},)
    sample_final_keep: Optional[int] = field(
        default=0,
        metadata={"help": "number of sampled point prompts will keep for LM"},)
    sample_final_nms_thr: Optional[float] = field(
        default=0.,
        metadata={"help": "enable nms or not"},)
    sam_feature_type: Optional[str] = field(
        default="mask",
        metadata={"help": "what type feature to use"},)
...
class UniGPTmodel(BaseFairseqModel):
        ...
        if model.img_model is not None:
            for p_name, p in model.img_model.named_parameters():
                if args.no_freeze_all:
                    ...
                    p.requires_grad = False
            # pdb.set_trace()
            # [p_name for p_name, p in model.img_model.named_parameters() if p.requires_grad]
        # freeze audio model
    ...
    def forward(self, src_tokens, 
        ...
        if self.ft_type == 1:
            ...
            x, extra = self.text_model(mlm_src_tokens, features_only=True)
    ...
    def get_image_representation(self, img_src_tokens):
        ...
        elif self.args.image_encoder == "sam":
            img_output = self.img_model(img_src_tokens)
        elif self.args.image_encoder == "b3-large":
            img_output = self.img_model(textual_tokens=None, visual_tokens=img_src_tokens, vision_masked_position=None)["encoder_out"]
            img_output = F.normalize(img_output, dim=-1)
        elif self.args.image_encoder == "b3-base":
            img_output = self.img_model(textual_tokens=None, visual_tokens=img_src_tokens, vision_masked_position=None)["encoder_out"]
            img_output = F.normalize(img_output, dim=-1)
        elif ',' in self.args.image_encoder:
            # pdb.set_trace()
            model_names = self.args.image_encoder.split(',')
            img_outputs = []
            for idx in range(len(model_names)):
                img_src_tokens_now = img_src_tokens.clone()
                bsz = img_src_tokens_now.size(0)
                if model_names[idx] == 'clip':
                    # resize the image to 224x224
                    img_src_tokens_now = F.interpolate(img_src_tokens_now, size=(224, 224), mode='bicubic')
                img_output = self.img_model[idx](img_src_tokens_now) # [257, bsz, 1024]
                src_len = img_output.size(0)
                img_output = img_output.transpose(0, 1) # T x B x C -> B x T x C [bsz, 257, 1024]
                img_output = img_output.reshape(-1, img_output.size(-1)) # [bsz x 257, 1024]
                img_output = self.img_connector[idx](img_output, src_len=src_len)
                img_output_reshape = img_output.reshape(bsz, -1, img_output.size(1))
                img_outputs.append(img_output_reshape)
                    
            # pdb.set_trace()
            img_outputs = torch.cat(img_outputs, dim=1).reshape(-1, img_output.size(1))
            return img_outputs
    ...
    def get_audio_representation(self, aud_src_tokens, aud_mask):
        ...
        fbank = aud_src_tokens.view(-1, aud_src_tokens.size(2), aud_src_tokens.size(3)) # B * Seg size * Fbank size * Dim -> B * Seg size * Fbank size * Dim
    ...
    def load_image_model(cls, args, task):
        ...
        def build_backbone_sam(args, visual_model_name, visual_pretrained):
            from segment_anything import sam_model_registry, SamResampler
            backbone = sam_model_registry[visual_model_name](checkpoint=visual_pretrained)
            model = SamResampler(backbone, args.sam_sample_mode, args.sample_grid_points, 
                                 feature_type=args.sam_feature_type)
            return model
        ...
        if args.image_encoder == 'sam':
            # pdb.set_trace()
            model = build_backbone_sam(args, args.visual_model_name, args.visual_pretrained)
            connector = build_connector(args, args.visual_output_dim, args.decoder_embed_dim)
            return model, connector
        
        if ',' in args.image_encoder:
            assert ',' in args.visual_model_name
            assert ',' in args.connector
            model_names = args.image_encoder.split(',')
            visual_model_names = args.visual_model_name.split(',')
            visual_pretraineds = args.visual_pretrained.split(',')
            connector_names = args.connector.split(',')
            visual_output_dims = args.visual_output_dim_str.split(',')
            visual_output_dims = [int(i) for i in visual_output_dims]
            model_list = []
            connector_list = []
            for model_name, visual_model_name, visual_pretrained, connector_name, visual_output_dim in zip(
                model_names, visual_model_names, visual_pretraineds, connector_names, visual_output_dims):
                if model_name == "clip":
                    model_list.append(build_backbone_clip(args, visual_model_name, visual_pretrained))
                elif model_name == "sam":
                    model_list.append(build_backbone_sam(args, visual_model_name, visual_pretrained))
                else:
                    raise NotImplementedError("Unknown model name {}".format(model_name))
                connector_list.append(build_connector2(connector_name, visual_output_dim, args.decoder_embed_dim, args))
            model_list = nn.ModuleList(model_list)
            connector_list = nn.ModuleList(connector_list)
            return  model_list, connector_list
        ...
        if args.image_encoder == "b3-large":
            ...
            num_heads = 16
            mlp_ratio = 4
            encoder_args = EncoderConfig(vocab_size=64010, multiway=True, layernorm_embedding=False, no_output_layer=True, drop_path_rate=drop_path_rate, encoder_embed_dim=embed_dim, encoder_attention_heads=num_heads, encoder_ffn_embed_dim=int(embed_dim*mlp_ratio), encoder_layers=depth)
            encoder_args.flash_attention = True
            ...
            print('load B3 large from', args.visual_pretrained)

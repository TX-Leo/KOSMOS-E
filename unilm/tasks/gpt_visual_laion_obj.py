...
@dataclass
class ImageGPTObjPretrainingConfig(GPTPretrainingConfig, WdsLoaderConfig):
    ...
    # data_weights: str = field(default="4,32,1", metadata={"help": "wild,laion,gpt"}) # not support now
...
@register_task("image_gpt_pretraining_laion_obj", dataclass=ImageGPTObjPretrainingConfig)
class ImageGPTLaionObjTask(GPTTask):
    ...
    def get_batch_iterator(
        ...
        concat_loader = ConcatLoader([lain_vl_loader, lm_loader])
        # concat_loader = ConcatLoader([lain_vl_loader])
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                vl_laion_loss, sample_size, logging_output = criterion(model, sample["vl_laion"], loss_name="image_laion")
        if ignore_grad:
            vl_laion_loss *= 0
        with torch.autograd.profiler.record_function("backward"):
            if isinstance(model, DeepSpeedEngine):
                model.backward(vl_laion_loss)
            else:
                optimizer.backward(vl_laion_loss)
        
        agg_loss += vl_laion_loss.detach().item()
        ...
        # disbale lm dataloader for debug
        if "gpt" not in sample:

...
from unilm.data.vl2.laion2b_obj_random_interleave_loader import Laion2BObjRandomInterleaveLoader
...
class ImageGPTPretrainingConfig(GPTPretrainingConfig, WdsLoaderConfig):
    ...
    locate_special_token: int = field(default=0, metadata={"help": "used to represent whether use <grounding> token"})
    phrase_mode: str = field(default="phrase", metadata={"help": "mode in phrase,expression"})
    
    # interleave
    laion_interleave_data_dir: str = field(default="", metadata={"help": ""})
    laion_interleave_batch_size: int = field(default=32, metadata={"help": ""})
    random_interleave_nums: int = field(default=2, metadata={"help": "used haw many pairs for one interleave sample"})
    
    data_weights: str = field(default="4,4,32,1", metadata={"help": "wild,laion_randinter,laion,gpt"})
...
@register_task("image_gpt_pretraining_wild_laion_v3_obj_randinter", dataclass=ImageGPTPretrainingConfig)
class ImageGPTWildLaionV3ObjRandInterTask(GPTTask):
    ...
    def get_batch_iterator(
        ...
        laion_interleave_dataset = Namespace(**{
            'data': json.load(open(f'{self.cfg.laion_interleave_data_dir}/json/train.json')),
            'data_dir': self.cfg.laion_interleave_data_dir,
            'shuffle': True})

        laion_interleave_vl_loader = Laion2BObjRandomInterleaveLoader(
            self.cfg,
            laion_interleave_dataset,
            self.dictionary,
            self.tokenizer,
            max_tokens=max_tokens,
            max_sentences=self.cfg.laion_interleave_batch_size,
            max_positions=max_positions,
            ignore_invalid_inputs=ignore_invalid_inputs,
            required_batch_size_multiple=required_batch_size_multiple,
            seed=seed,
        ...
        logger.info(f"dataset types: wild, laion_randinter, laion, gpt")
        logger.info(f"dataset weights: {data_weight}")
        
        # wild,laion_randinter,laion,gpt
        concat_loader = MixLoader([vl_loader, laion_interleave_vl_loader, lain_vl_loader, lm_loader], data_weight)
    ...
    def train_step(
        ...
        if 'vl' in sample:
            ...
            loss_key = 'vl_laion'
        elif 'vl_laion_interleave' in sample:
            loss_name = "image_laion_interleave"
            loss_key = 'vl_laion_interleave'
        elif 'gpt' in sample:
            loss_name = "gpt"
            loss_key = 'gpt'
        else:

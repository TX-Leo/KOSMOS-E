...
@register_task("image_gpt_pretraining_wild_laion", dataclass=ImageGPTPretrainingConfig)
class ImageGPTWildLaionTask(GPTTask):
    ...
    def get_batch_iterator(
        ...
        concat_loader = ConcatLoader([vl_loader, lain_vl_loader, lm_loader])
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                vl_loss, sample_size, logging_output = criterion(model, sample["vl"], loss_name="image_wild")
        if ignore_grad:
            vl_loss *= 0
...
@register_task("image_gpt_pretraining_vlm_only_wild_laion", dataclass=ImageGPTPretrainingConfig)
class OnlyVLMImageGPTWildLaionTask(ImageGPTWildLaionTask):

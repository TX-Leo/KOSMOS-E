...
class ImageGPTPretrainingConfig(GPTPretrainingConfig, WdsLoaderConfig):
    tokens_per_sample4vlm: int = field(default=77, metadata={"help": ""})
    image_feature_length: int = field(default=196, metadata={"help": ""})
    max_sentences4vlm: int = field(default=256, metadata={"help": "vlm bsz"})


@register_task("image_gpt_pretraining", dataclass=ImageGPTPretrainingConfig)
class ImageGPTTask(GPTTask):
    ...
    def get_batch_iterator(
        ...
        image_size = self.vlm_model.img_model.visual.image_size
        vl_loader = VlmLoader(self.cfg, dataset, self.dictionary, self.tokenizer, max_tokens=max_tokens, max_sentences=self.cfg.max_sentences4vlm, max_positions=max_positions, ignore_invalid_inputs=ignore_invalid_inputs,
                              required_batch_size_multiple=required_batch_size_multiple, seed=seed, epoch=epoch, num_shards=num_shards, shard_id=shard_id, nested_batch_key=None, image_size=image_size)
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                vl_loss, sample_size, logging_output = criterion(model, sample["image"], loss_name="image")
        if ignore_grad:
            vl_loss *= 0
...
@register_task("image_gpt_pretraining_vlm_only", dataclass=ImageGPTPretrainingConfig)
class OnlyVLMImageGPTTask(ImageGPTTask):

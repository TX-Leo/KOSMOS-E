...
OUTPUT_PATH=$OUTPUT_PATH/$TASK/$FT_TYPE-$N_EPOCH-$WARMUP_RATIO-$BSZ-$LR-$SEED
mkdir -p $OUTPUT_PATH
echo $OUTPUT_PATH
...
python -m torch.distributed.launch --nproc_per_node=$N_GPU train.py $TASK_DATA_DIR --fp16 --fp16-init-scale 4 --threshold-loss-scale 1 --fp16-scale-window 128 \
    ...
    --gpt-dict /mnt/msranlp/yaru/data/openwebtext2/dict.txt  \
    --mlm-dict /mnt/msranlp/yaru/data/roberta_model/roberta.base/dict.txt --ft-type $FT_TYPE \
    --validate-interval-updates $VALIDATE_INTERVAL ${extra_args} |& tee $OUTPUT_PATH/train_log.txt \

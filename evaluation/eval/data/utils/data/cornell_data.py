...
Brief: 
'''
from data.utils.dataset_processing import grasp, image
...
#  ======================================================
    ...
    def show_rgb_Image_and_Grasps(self,rgb_Image,Grasps,vis=True,save_path=None):
        import cv2
    ...
    @classmethod
    def show_rgb_Image_and_predicted_Grasps_and_real_Grasps(self,rgb_Image,predicted_Grasps,real_Grasps,vis=True,save_path=None):
        import cv2
        # show all grasp in an image
        if vis or save_path is not None:
            if isinstance(rgb_Image,image.Image):
            ...
            for predicted_grasp_rectangle in predicted_Grasps.as_grs:
                ...
                cv2.line(img,(round(points[0][0]),round(points[0][1])),(round(points[1][0]),round(points[1][1])),(0,0,0),1)
                cv2.line(img,(round(points[1][0]),round(points[1][1])),(round(points[2][0]),round(points[2][1])),(0,0,255),2)
                cv2.line(img,(round(points[2][0]),round(points[2][1])),(round(points[3][0]),round(points[3][1])),(0,0,0),1)
                cv2.line(img,(round(points[3][0]),round(points[3][1])),(round(points[0][0]),round(points[0][1])),(0,0,255),2)
    ...
    def dataset_multiobject(self,new_dataset_path,idx_start=0,idx_end=14160,instructions_file_name='instructions_temp_1',read_idxs_from_file=False,overlap_width=15):
        ...
        for i in range(0, len(idxs), 4):
            ...
            for j in range(4):
                ...
                # name
                instruction_name = self.instructions_name[idx]
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_name.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_name)
                # shape
                instruction_shape = self.instructions_shape[idx]
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_shape.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_shape)
                # purpose
                instruction_purpose = self.instructions_purpose[idx]
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_purpose.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_purpose)
                # strategy
                instruction_strategy = self.instructions_strategy[idx]
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_strategy.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_strategy)
                # color
                instruction_color = self.instructions_color[idx]
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_color.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_color)
                # part
                instruction_part = self.instructions_part[idx]
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_part.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_part)
                # position
                instruction_position = self.get_instruction_position_for_multiobject(j)
                self.generate_grasp_xywha_tsv(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_position.tsv'),None,'xya',encoded=True,decimals=3,instruction=instruction_position)
                # angle
                instruction_angle = self.instructions_angle[idx]
                self.generate_grasp_xywha_tsv_with_instruction_angle(rgb_image_base64,Grasps,new_Grasps_path.replace('grasp_xywha.txt','grasp_xya_encoded_with_instruction_angle.tsv'),None,'xya',encoded=True,decimals=3)
    ...
    def evaluate(self,dataloader_num,generate_predictions_func,iou_threshold=0.25,grasp_format='xywha',splited=False,encoded=False,suffix=None,vis=False,save_dir=None,instruction_type=None):
        ...
        test_idxs_txt_path = f'/mnt/msranlpintern/dataset/cornell-instruction-v1/dataloader/{dataloader_num}/dataloader_config/json/test.txt'
    ...
    # #================================================================
    # def eval_Grasp_iou(self,predicted_Grasp,real_Grasps,iou_threshold=0.25):
    #     max_iou = predicted_Grasp.max_iou(real_Grasps)
    #     if max_iou > iou_threshold:
    #         print(f'====== [max_iou:{max_iou}] It is a correct grasp ======')
    #     else:
    #         print(f'====== [max_iou:{max_iou}] It is a failed grasp ======')
    #     return max_iou
    
    # def filter_data(self):
    #     pass

    # def generate_data_encoded(self,bins=256):
    #     from sklearn.preprocessing import KBinsDiscretizer
    #     import matplotlib.pyplot as plt
    #     import numpy as np
    #     import os
        
    #     if os.path.exists(f'{self.dataset_root}/all_data/all_x.txt'):
    #         return
    #     else:
    #         os.makedirs(f'{self.dataset_root}/all_data/')
    #     def save_all_data(data,save_path):
    ...
    #     def plot_all_data(data,save_path,bins,color='red'): 
    ...
    #         print(f'====== plot successfully: {save_path}')
    ...
    #     for idx in range(self.image_sum):
    #         grasps = grasp.Grasps.load_from_cornell_file(self.grasp_xywha_files[idx])
    #         all_x.extend(grasps.get_all_x())
    #         all_y.extend(grasps.get_all_y())
    #         all_w.extend(grasps.get_all_w())
    #         all_h.extend(grasps.get_all_h())
    #         all_a.extend(grasps.get_all_a())
        
    #     all_x_encoded = [int(x) for x in KBinsDiscretizer(n_bins=bins,encode='ordinal',strategy='quantile').fit_transform(np.array(all_x).reshape(-1,1))]
    #     all_y_encoded = [int(x) for x in KBinsDiscretizer(n_bins=bins,encode='ordinal',strategy='quantile').fit_transform(np.array(all_y).reshape(-1,1))]
    #     all_w_encoded = [int(x) for x in KBinsDiscretizer(n_bins=bins,encode='ordinal',strategy='quantile').fit_transform(np.array(all_w).reshape(-1,1))]
    #     all_h_encoded = [int(x) for x in KBinsDiscretizer(n_bins=bins,encode='ordinal',strategy='quantile').fit_transform(np.array(all_h).reshape(-1,1))]
    #     all_a_encoded = [int(x) for x in KBinsDiscretizer(n_bins=bins,encode='ordinal',strategy='quantile').fit_transform(np.array(all_a).reshape(-1,1))]
        
    #     save_all_data(all_x,f'{self.dataset_root}/all_data/all_x.txt')
    #     save_all_data(all_y,f'{self.dataset_root}/all_data/all_y.txt')
    #     save_all_data(all_w,f'{self.dataset_root}/all_data/all_w.txt')
    #     save_all_data(all_h,f'{self.dataset_root}/all_data/all_h.txt')
    #     save_all_data(all_a,f'{self.dataset_root}/all_data/all_a.txt')
        
    #     plot_all_data(all_x,f'{self.dataset_root}/all_data/all_x.png',bins=100,color='red')
    #     plot_all_data(all_y,f'{self.dataset_root}/all_data/all_y.png',bins=100,color='red')
    #     plot_all_data(all_w,f'{self.dataset_root}/all_data/all_w.png',bins=100,color='red')
    #     plot_all_data(all_h,f'{self.dataset_root}/all_data/all_h.png',bins=100,color='red')
    #     plot_all_data(all_a,f'{self.dataset_root}/all_data/all_a.png',bins=100,color='red')
        
    #     save_all_data(all_x_encoded,f'{self.dataset_root}/all_data/all_x_encoded.txt')
    #     save_all_data(all_y_encoded,f'{self.dataset_root}/all_data/all_y_encoded.txt')
    #     save_all_data(all_w_encoded,f'{self.dataset_root}/all_data/all_w_encoded.txt')
    #     save_all_data(all_h_encoded,f'{self.dataset_root}/all_data/all_h_encoded.txt')
    #     save_all_data(all_a_encoded,f'{self.dataset_root}/all_data/all_a_encoded.txt')
        
    #     plot_all_data(all_x_encoded,f'{self.dataset_root}/all_data/all_x_encoded.png',bins=bins,color='blue')
    #     plot_all_data(all_y_encoded,f'{self.dataset_root}/all_data/all_y_encoded.png',bins=bins,color='blue')
    #     plot_all_data(all_w_encoded,f'{self.dataset_root}/all_data/all_w_encoded.png',bins=bins,color='blue')
    #     plot_all_data(all_h_encoded,f'{self.dataset_root}/all_data/all_h_encoded.png',bins=bins,color='blue')
    #     plot_all_data(all_a_encoded,f'{self.dataset_root}/all_data/all_a_encoded.png',bins=bins,color='blue')
    
    # def generate_grasp_tsv_encoded(self,idx_start,idx_end):
    #     import numpy as np
    #     def open_all_data(txt_file_path):
    #         with open(txt_file_path,'r') as txt_file:
    #             lines = txt_file.readlines()
    #             data = np.array([int(line.strip()) for line in lines])
    #         txt_file.close()
    #         return data 
    #     all_x_encoded = open_all_data(f'{self.dataset_root}/all_data/all_x_encoded.txt')
    #     all_y_encoded = open_all_data(f'{self.dataset_root}/all_data/all_y_encoded.txt')
    #     all_w_encoded = open_all_data(f'{self.dataset_root}/all_data/all_w_encoded.txt')
    #     all_h_encoded = open_all_data(f'{self.dataset_root}/all_data/all_h_encoded.txt')
    #     all_a_encoded = open_all_data(f'{self.dataset_root}/all_data/all_a_encoded.txt')
    #     Grasps = grasp.Grasps()
    #     for idx in range(idx_start,idx_end):
    #         if idx>=self.image_sum:
    #             break
    #         pointer_start=self.pointer[idx-1] if idx!=0 else 0
    #         pointer_end=self.pointer[idx]
    #         for i in range(pointer_start,pointer_end):
    #             Grasps.append(grasp.Grasp((all_x_encoded[i],all_y_encoded[i]),all_w_encoded[i],all_h_encoded[i],all_a_encoded[i]))
    #         Grasps.save_txt(self.grasp_xywha_files[idx].replace('grasp.txt','grasp_encoded.txt'))
            

    #         relative_pc_npy_file_path = self.pc_npy_files[idx].replace(self.dataset_root,'/')
    #         # print(f'{self.pc_npy_files[idx]}')
    #         # print(f'{self.dataset_root}')
    #         # print(f'{relative_pc_npy_file_path}')
    #         rgb_Image = image.Image.from_file(self.rgb_files[idx])
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_xywha_encoded.tsv'),relative_pc_npy_file_path,xywha='xywha')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_xywa_encoded.tsv'),relative_pc_npy_file_path,xywha='xywa')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_xyha_encoded.tsv'),relative_pc_npy_file_path,xywha='xyha')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_xya_encoded.tsv'),relative_pc_npy_file_path,xywha='xya')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_xy_encoded.tsv'),relative_pc_npy_file_path,xywha='xy')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_w_encoded.tsv'),relative_pc_npy_file_path,xywha='w')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_h_encoded.tsv'),relative_pc_npy_file_path,xywha='h')
    #         self.generate_grasp_tsv(rgb_Image,Grasps,self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_a_encoded.tsv'),relative_pc_npy_file_path,xywha='a')
    #         print(f'======[idx_{idx} sum_{self.pointer[idx]}] grasp_tsv_encoded is generated successfully')
    ...
    # def eval_Grasp_numerical(self,predicted_Grasp,real_grasp_Rectangles):
    #     bias_dict = real_grasp_Rectangles.as_grasps.get_numerical_bias(predicted_Grasp)
    #     print(f'====== Bias Dict:\n{bias_dict}')
    #     return bias_dict
    
    # def eval_Grasps_numerical(self,predicted_Grasps,real_grasp_Rectangles):
    #     for predicted_Grasp in predicted_Grasps:
    #         bias_dict = self.eval_Grasp_numerical(predicted_Grasp,real_grasp_Rectangles)
    
    # def gen_instruction_tsv(self,idxs):
    #     import base64
    #     import csv
    #     from PIL import Image
    #     import io
        
    #     def image_numpy_to_base64(img_numpy):
    #         # Numpy to PIL.Image
    #         img_pil = Image.fromarray(img_numpy)
    #         # PIL.Image to BytesIO
    #         img_byte_array = io.BytesIO()
    ...
    #     for idx in idxs:
    #         txt_file_path = self.grasp_tsv_files[idx].replace('grasp.tsv','instruction_name.txt')
    #         rgb_Image = self.rgb_files[idx]
    #         rgb_image_base64 = image_numpy_to_base64(rgb_Image.img)
    #         (rgb_image_height,rgb_image_width,_) = rgb_Image.img.shape 
    #         with open(txt_file_path,'r') as txt_file:
    #             instruction = txt_file.readline()
    #         tsv_file_path = self.grasp_tsv_files[idx].replace('grasp.tsv','grasp_instruction_name.tsv')
    #         data = ['00000000000',instruction,rgb_image_base64,rgb_image_width,rgb_image_height]
    ...
    def test_multiobject(self):
        grasp_Rectangles = grasp.GraspRectangles.load_from_cornell_file(r'G:\dataset\multiObject-v1\00\pcd0000cpos.txt')
        rgb_Image = image.Image.from_file(r'G:\dataset\multiObject-v1\00\pcd0000r.jpg')
        self.show_rgb_Image_and_grasp_Rectangles(rgb_Image,grasp_Rectangles)
